...
export default function Landing() {
  ...
  return (
    <div className="w-full max-w-full space-y-10 overflow-x-hidden pb-20 sm:space-y-14">
      {/* Hero Section */}
      <section className="relative overflow-hidden rounded-3xl border border-orange-500/20 bg-[linear-gradient(130deg,#07070a_0%,#111827_50%,#1f2937_100%)] p-3.5 sm:p-6 md:p-8">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1.2fr_1fr] lg:gap-8">
          <div className="space-y-4 sm:space-y-5">
            ...
            <motion.h1
              ...
              className="text-[2.15rem] font-black leading-[1.02] tracking-tight text-white sm:text-4xl md:text-6xl"
            >
              Don’t Just Study.
            ...
            <motion.p
              ...
              className="max-w-2xl text-[0.95rem] text-zinc-300 sm:text-base"
            >
            ...
            >
              {[
                ...
                <div key={item.label} className="rounded-xl border border-white/10 bg-white/5 px-2 py-1.5 text-center backdrop-blur sm:rounded-2xl sm:px-3 sm:py-2">
                  <p className="text-[1.45rem] font-black text-white sm:text-lg">{item.val}</p>
            ...
            >
              <Link
                to="/auth"
                className="group inline-flex items-center justify-center gap-2 rounded-xl bg-orange-500 px-5 py-2.5 text-sm font-bold text-white transition-all duration-300 ease-out hover:-translate-y-0.5 hover:bg-orange-600"
              >
              ...
              <Link
                to="/test-series"
                className="inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/10 px-5 py-2.5 text-sm font-bold text-white backdrop-blur transition-all duration-300 ease-out hover:bg-white/20"
              >
                Explore Test Series
              </Link>
            </motion.div>
            ...
            >
              ...
              <input
                ...
                className="h-11 w-full rounded-xl bg-transparent pl-10 pr-3 text-sm text-white outline-none placeholder:text-zinc-500 sm:h-12 sm:pr-28"
              />
              <button
                type="submit"
                className="mt-2 w-full rounded-lg bg-orange-500 px-4 py-2 text-sm font-bold text-white transition-all duration-300 ease-out hover:bg-orange-600 sm:absolute sm:right-2 sm:top-1/2 sm:mt-0 sm:w-auto sm:-translate-y-1/2"
              >
                Search
              </button>
            </motion.form>
            ...
            <div className="grid grid-cols-3 gap-2">
              {heroHighlights.map(({ label, icon: Icon }, index) => (
                <motion.div
                  ...
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.24 + index * 0.08, duration: 0.36, ease: "easeOut" }}
                  className="rounded-xl border border-white/10 bg-white/5 p-2.5 text-center transition-all duration-300 ease-out hover:bg-white/10"
                >
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
              {heroFeatures.map(({ title, subtitle, icon: Icon }, idx) => (
                <motion.div
                  ...
                  className="rounded-2xl border border-white/10 bg-white/5 p-3 transition-all duration-300 ease-out hover:bg-white/10"
                >
                  <motion.div
                    animate={{ y: [0, -3, 0] }}
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-6">
          {categories.map((cat, idx) => (
            <motion.div
              ...
              className="group relative cursor-pointer overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/60 p-3.5 text-center transition-all duration-300 ease-out hover:border-orange-500/50"
            >
